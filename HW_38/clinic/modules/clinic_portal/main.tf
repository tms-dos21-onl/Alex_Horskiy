locals {
  app_name = try(var.app_name, "clinic-portal")
}

resource "google_service_account" "main" {
  account_id   = "${local.app_name}-id"
  display_name = "Service Account for Clinic.Portal"
}

resource "google_compute_instance_template" "main" {
  name         = "${local.app_name}-template"
  description  = "This template is used to create Clinic.Portal instances."
